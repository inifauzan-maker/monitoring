...
<aside class="navbar navbar-vertical navbar-expand-lg">
    <div class="container-fluid">
        ...
        <a class="navbar-brand py-3" href="{{ route('dashboard.beranda') }}">
            ...
            <ul class="navbar-nav pt-lg-3">
                @foreach ($bagianMenu as $indeksBagian => $bagian)
                    ...
                    @foreach ($itemTerlihat as $item)
                        ...
                        @if ($punyaAnak)
                            ...
                            <li class="nav-item">
                                <a class="nav-link dropdown-toggle {{ $apakahAktif ? 'active' : '' }}" href="#{{ $idSubmenu }}"
                                    data-bs-toggle="collapse" role="button" aria-expanded="{{ $apakahAktif ? 'true' : 'false' }}"
                                ...
                                <div class="collapse {{ $apakahAktif ? 'show' : '' }}" id="{{ $idSubmenu }}">
                                    <ul class="navbar-nav ps-3">
                                        @foreach ($item['anak'] as $anak)
                                            ...
                                            <li class="nav-item">
                                                <a class="nav-link {{ $anakAktif ? 'active' : '' }}" href="{{ route($anak['route']) }}">
                                                    <span class="nav-link-icon d-md-none d-lg-inline-block">
                        ...
                        @elseif ($punyaRoute)
                            <li class="nav-item">
                                <a class="nav-link {{ $apakahAktif ? 'active' : '' }}" href="{{ route($item['route']) }}">
                                    <span class="nav-link-icon d-md-none d-lg-inline-block">
                                        @include('komponen.ikon_menu', ['ikon' => $item['ikon']])
                                    </span>
                                    <span class="nav-link-title">{{ $item['judul'] }}</span>
